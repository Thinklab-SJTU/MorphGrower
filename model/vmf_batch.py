# taken from https://github.com/jiacheng-xu/vmf_vae_nlp/blob/master/NVLL/distribution/vmf_batch.py
# Spherical Latent Spaces for Stable Variational Autoencoders.
# Jiacheng Xu and Greg Durrett. EMNLP 2018.
# adjustements from Sophie Laturnus Jan 2021
...
class vMF(torch.nn.Module):
    def __init__(self, lat_dim, kappa=1, device=default_device):
        """
        ...
        self.kld = GVar(torch.from_numpy(vMF._vmf_kld(kappa, lat_dim)).float(), self.device)
    ...
    def sample_cell(self, mu, norm, kappa):
        ...
        # batch version
        w_var = GVar(w * torch.ones(batch_sz, lat_dim), self.device)
        v = self._sample_ortho_batch(mu, lat_dim)
        scale_factr = torch.sqrt(
            GVar(torch.ones(batch_sz, lat_dim), self.device) - torch.pow(w_var, 2))
    ...
    def _sample_ortho_batch(self, mu, dim):
        ...
        v = GVar(torch.randn(_batch_sz, dim, 1), self.device)  # TODO random
    ...
    def _sample_orthonormal_to(self, mu, dim):
        ...
        """
        v = GVar(torch.randn(dim), self.device)  # TODO random
